...
function OtherPageNavBar() {
  ...
  return (
    <nav className="bg-gray-500 absolute top-0 w-full z-50">
      <div className="container mx-auto flex justify-between items-center px-4 lg:px-8 py-4">
        ...
        <ul className={`lg:flex space-x-4 ${isOpen ? 'block' : 'hidden'}`}>
          <li className="hover:bg-orange-700 rounded px-2 py-1">
            <Link to="/" className="text-white">{output.home}</Link>
          </li>
          <li className="hover:bg-orange-700 rounded px-2 py-1">
            <Link to="/menu" className="text-white">{output.menu}</Link>
          </li>
          <li className="hover:bg-orange-700 rounded px-2 py-1">
            <Link to="/delivery" className="text-white">{output.delivery}</Link>
          </li>
          <li className="hover:bg-orange-700 rounded px-2 py-1">
            <Link to="/findus" className="text-white">{output.findus}</Link>
          </li>
          <li className="hover:bg-orange-700 rounded px-2 py-1">
        ...
        <div className={`lg:hidden fixed inset-y-0 right-0 flex flex-col justify-center bg-gray-800 w-64 z-50 transform ${isOpen ? 'translate-x-0' : 'translate-x-full'} transition duration-300 ease-in-out rounded-l-lg`}>
          <ul className="text-white text-center">
            <li className="hover:bg-orange-700 rounded py-2">
              <Link to="/" onClick={toggleMenu} className="text-white">{output.home}</Link>
            </li>
            <li className="hover:bg-orange-700 rounded py-2">
              <Link to="/menu" onClick={toggleMenu} className="text-white">{output.menu}</Link>
            </li>
            <li className="hover:bg-orange-700 rounded py-2">
              <Link to="/delivery" onClick={toggleMenu} className="text-white">{output.delivery}</Link>
            </li>
            <li className="hover:bg-orange-700 rounded py-2">
              <Link to="/findus" onClick={toggleMenu} className="text-white">{output.findus}</Link>
            </li>
            <li className="hover:bg-orange-700 rounded px-2 py-1">
            ...
            <li className="hover:bg-orange-700 rounded px-2 py-1">
              <button onClick={() => {dispatch(changeLanguageNav('cn'));dispatch(changeLanguageHome('cn'));dispatch(changeLanguage('cn'));dispatch(changeLanguageFindUs('cn'));dispatch(changeLanguageDelivery('cn'));toggleMenu()}} className="focus:outline-none">
                <img src={`${path}cn.svg`} alt="Chinese" className="h-4 w-6" />
              </button>
            </li>
          </ul>
